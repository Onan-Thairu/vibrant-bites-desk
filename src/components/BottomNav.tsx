...
const trainerNavItems: NavItem[] = [
  ...
  { icon: TrendingUp, label: "Employees", path: "/trainer/employees" },
  { icon: Settings, label: "Settings", path: "/settings" },
];

const employeeNavItems: NavItem[] = [
  { icon: Home, label: "Home", path: "/employee" },
  { icon: Utensils, label: "Meal Plans", path: "/employee/plans" },
  { icon: TrendingUp, label: "Progress", path: "/employee/progress" },
  { icon: Settings, label: "Settings", path: "/settings" },
];

interface BottomNavProps {
  userRole: "trainer" | "employee";
}

export function BottomNav({ userRole }: BottomNavProps) {
  const location = useLocation();
  const navItems = userRole === "trainer" ? trainerNavItems : employeeNavItems;
